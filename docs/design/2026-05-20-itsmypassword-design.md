...
**Inspirations:** [LessPass](https://lesspass.com) (algorithm) and [Spectre](https://spectre.app) (template-based memorable passwords).
...
| Master password (at rest, PIN mode) | Local disk compromise | Encrypted with Argon2id-derived key from a user PIN; clear warning shown when opt-in |
...
The algorithm is deliberately close to LessPass v2 (proven, auditable) with **one upgrade**: Argon2id instead of PBKDF2-SHA256.
...
```
salt    = utf8( domain || email || hex(counter) )       // LessPass-style, no separator
...
> **Why Argon2id?** OWASP currently recommends Argon2id over PBKDF2 for new designs; it is memory-hard, slowing GPU/ASIC attacks if a single derived password is leaked. We ship `hash-wasm` (~30 KB WASM) and load it only inside the service worker. If WASM fails to load (very rare on modern Chrome), the extension refuses to generate rather than silently falling back to a weaker algorithm — see § 12 open questions.

### 3.3 Rendering — Random Characters mode

Direct port of LessPass's `consume_entropy` / `get_one_char_per_rule` / `insert_string_pseudo_randomly` (see [LessPass source](https://github.com/lesspass/lesspass)).
...
- **Per-word selection:** `consume_entropy` with the 7,776-word pool.
...
- Session state — unlocked master in `chrome.storage.session` with `setAccessLevel('TRUSTED_CONTEXTS')` (content scripts can not read it).
...
Responsibilities:
- Detect `input[type=password]` (incl. shadow DOM via open-shadow polyfill, dynamic mounts via `MutationObserver`).
- Identify nearest username/email field (heuristics adapted from Bitwarden autofill — see § 7).
...
3. **Shadow DOM:** traverse open shadow roots; skip closed roots (unfixable from the outside).
...
Implementation reference: Bitwarden `apps/browser/src/autofill` (GPL-3.0). **We will not copy code** — only the heuristic ideas — to remain MIT-licensable. See § 12 for licence audit task.
...
| `test` | PR, push | Vitest with coverage; **golden-vector tests** against published LessPass outputs (regression guard) |
...
│   │   │   ├── render.ts            # consume_entropy + rules
...
1. **Bitwarden licence audit.** Bitwarden autofill is GPL-3.0. We must not copy code; we may study heuristics and reimplement. To be confirmed by a quick legal-read pass before content-script work starts.
2. **WASM bundling.** Decide whether to embed Argon2 WASM inline (base64) or ship as a separate `.wasm` asset (cleaner but requires CSP `'wasm-unsafe-eval'`).
3. **Firefox parity.** Manifest V3 for Firefox is stable in 2025; we plan parity but ship Chrome first. Document any divergence.
4. **Master strength enforcement.** zxcvbn is ~400 KB; consider a lighter alternative or load it lazily only at first-run.
5. **Migration story.** Once v1 ships, any change to the algorithm or salt is a breaking change for every user's stored passwords. We freeze the algorithm in v1.0.0 and only ever add new modes — never modify existing ones.
